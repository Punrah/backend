...
{
	$q=mysql_query("INSERT INTO m_anggota VALUES('$_POST[no]','$_POST[nama]','$_POST[alamat]','$_POST[jk]','$_POST[telp]')")or die(mysql_error());

	$idt=date("U");
	if($_POST['jenis']=='harian')
	{
		$bayar=10000;
		$q2=mysql_query("INSERT INTO t_anggota VALUES('$idt','$_POST[no]','$_POST[tgl]',date_add('$_POST[tgl]',INTERVAL 1 DAY),'$bayar','$_POST[jenis]')")or die(mysql_error());
	}
	elseif($_POST['jenis']=='bulanan')
	{
		$bayar=50000;
		$q2=mysql_query("INSERT INTO t_anggota VALUES('$idt','$_POST[no]','$_POST[tgl]',date_add('$_POST[tgl]', INTERVAL 30 DAY),'$bayar','$_POST[jenis]')")or die(mysql_error());
	}
	...
	if($q)
	{
		header("location:../../dashboard.php?m=anggota&hal=tambah&message=1");
	}
	else
	{
		header("location:../../dashboard.php?m=anggota&hal=tambah&message=2");
	}
...
function p_member()
{
	...
	$idt=date("U");
	$q=mysql_query("select * from m_harga_member");
	$d=mysql_fetch_array($q);
	if($_POST['jenis']=='harian')
	{

		$bayar=$d['harian'];
		$q2=mysql_query("INSERT INTO t_anggota VALUES('$idt','$_POST[id_anggota]','$_POST[tgl]','$_POST[tgl]','$bayar','$_POST[jenis]')")or die(mysql_error());
	}
	elseif($_POST['jenis']=='bulanan')
	{
		$bayar=$d['bulanan'];
...
function d_member()
{
	$q1=mysql_query("delete from t_anggota where id_transaksi='$_POST[id_transaksi]'")or die(mysql_error());
	if($q1)
	{
		header("location:../../dashboard.php?m=laporan&hal=harian&tgl=$_POST[tgl1]&ms=1");
	}
	else
	{
		header("location:../../dashboard.php?m=laporan&hal=harian&tgl=$_POST[tgl1]&ms=2");
	}
}
